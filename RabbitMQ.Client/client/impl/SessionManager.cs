...
namespace RabbitMQ.Client.Impl
{
    public class SessionManager
    {
        ...
        public Task AutoCloseConnection()
        {
            return m_connection.Abort(Constants.ReplySuccess, "AutoClose", ShutdownInitiator.Library, Timeout.Infinite);
        }
            ...
            if (m_autoClose)
            {
                lock (m_sessionMap)
                {
                    if (m_sessionMap.Count == 0)
                    {
                        ...
                        Task.Factory.StartNew(AutoCloseConnection);
                    }
                }
            }
        }
