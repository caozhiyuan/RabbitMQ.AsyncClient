...
namespace RabbitMQ.Client.Framing.Impl
{
    public class AutorecoveringConnection : IConnection, IRecoverable
    {
        ...
        protected readonly SemaphoreSlim recoverySlimTarget = new SemaphoreSlim(1, 1);
        ...
        protected readonly SemaphoreSlim m_modelsSlim = new SemaphoreSlim(1, 1);

        protected ConcurrentDictionary<RecordedBinding, byte> m_recordedBindings =
            new ConcurrentDictionary<RecordedBinding, byte>();

        protected List<AsyncEventHandler<ConnectionBlockedEventArgs>> m_recordedBlockedEventHandlers =
            new List<AsyncEventHandler<ConnectionBlockedEventArgs>>();
        ...
        protected List<AsyncEventHandler<EventArgs>> m_recordedUnblockedEventHandlers =
            new List<AsyncEventHandler<EventArgs>>();

        private AsyncEventHandler<ConsumerTagChangedAfterRecoveryEventArgs> m_consumerTagChange;
        private AsyncEventHandler<QueueNameChangedAfterRecoveryEventArgs> m_queueNameChange;
        private AsyncEventHandler<EventArgs> m_recovery;

        private AsyncEventHandler<ConnectionRecoveryErrorEventArgs> m_connectionRecoveryError;
        ...
        public event AsyncEventHandler<QueueNameChangedAfterRecoveryEventArgs> QueueNameChangeAfterRecovery
        {
            add
            {
                lock (m_eventLock)
        ...
        public void BeginAutomaticRecovery()
        {
            lock (recoveryLockTarget)
            {
                if (!performingRecovery)
                {
                    ...
                    recoveryTaskFactory.StartNew(async () =>
                    {
        ...
        public async Task<RecoveryAwareModel> CreateNonRecoveringModel()
        {
            ISession session = m_delegate.CreateSession();
            var result = new RecoveryAwareModel(session);
            await result._Private_ChannelOpen("");
            return result;
        }
        ...
        ///<summary>API-side invocation of connection abort.</summary>
        public async Task Abort(ushort reasonCode, string reasonText)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Abort(reasonCode, reasonText);
        }

        ///<summary>API-side invocation of connection abort with timeout.</summary>
        public async Task Abort(int timeout)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Abort(timeout);
        }

        ///<summary>API-side invocation of connection abort with timeout.</summary>
        public async Task Abort(ushort reasonCode, string reasonText, int timeout)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Abort(reasonCode, reasonText, timeout);
        }
        ...
        public async Task Close()
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Close();
        }

        ///<summary>API-side invocation of connection.close.</summary>
        public async Task Close(ushort reasonCode, string reasonText)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
               await m_delegate.Close(reasonCode, reasonText);
        }
        ...
        public async Task Close(int timeout)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Close(timeout);
        }

        ///<summary>API-side invocation of connection.close with timeout.</summary>
        public async Task Close(ushort reasonCode, string reasonText, int timeout)
        {
            this.ManuallyClosed = true;
            if (m_delegate.IsOpen)
                await m_delegate.Close(reasonCode, reasonText, timeout);
        }
        ...
        protected async Task RecoverBindings()
        {
        ...
        {
            List<AsyncEventHandler<ConnectionBlockedEventArgs>> handler = m_recordedBlockedEventHandlers;
            if (handler != null)
            {
                foreach (AsyncEventHandler<ConnectionBlockedEventArgs> eh in handler)
                {
        ...
        {
            foreach (AsyncEventHandler<ShutdownEventArgs> eh in m_recordedShutdownEventHandlers)
            {
        ...
        {
            List<AsyncEventHandler<EventArgs>> handler = m_recordedUnblockedEventHandlers;
            if (handler != null)
            {
                foreach (AsyncEventHandler<EventArgs> eh in handler)
                {
        ...
        protected async Task RecoverConsumers()
        {
            foreach (KeyValuePair<string, RecordedConsumer> pair in m_recordedConsumers)
            {
                ...
                try
                {
                    string newTag = await cons.Recover();
                    ...
                    {
                        foreach (AsyncEventHandler<ConsumerTagChangedAfterRecoveryEventArgs> h in m_consumerTagChange.GetInvocationList())
                        {
                            try
                            {
        ...
        protected async Task RecoverEntities()
        {
        ...
        protected async Task RecoverExchanges()
        {
        ...
        protected async Task RecoverModels()
        {
            await m_modelsSlim.WaitAsync();
            try
            {
                ...
                {
                    await m.AutomaticallyRecover(this, m_delegate);
                }
            }
            finally
            {
