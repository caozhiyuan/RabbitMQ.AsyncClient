...
namespace RabbitMQ.Client.Framing.Impl
{
    public class Connection : IConnection
    {
        ...
        private readonly bool m_insist;
        ...
        {
            return Close(reason, false, Timeout.Infinite);
        }
        ...
        ///</para>
        ///</remarks>
        public async Task Close(ShutdownEventArgs reason, bool abort, int timeout)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    await m_session0.Transmit(ConnectionCloseWrapper(reason.ReplyCode,
                        reason.ReplyText));
                }
...
#pragma warning restore 0168
        ...
        {
            InboundFrame frame = await m_frameHandler.ReadFrame();
        ...
        {
            await StartAndTune();
            await m_model0.ConnectionOpen(m_factory.VirtualHost, String.Empty, false);
        }
...
entry.ToString());
        ...
        ///</para>
        ///</remarks>
        public async Task QuiesceChannel(SoftProtocolException pe)
        {
            ...
            // installed above.
            await newSession.Transmit(ChannelCloseWrapper(pe.ReplyCode, pe.Message));
        }
        ...
        public async void HeartbeatReadTimerCallback(object state)
        {
            ...
            try
            {
                if (!m_closed)
                {
                    if (!(await m_heartbeatRead.WaitAsync(Timeout.InfiniteTimeSpan)))
                    {
                        m_missedHeartbeats++;
                    }
                    else
        ...
        public async void HeartbeatWriteTimerCallback(object state)
        {
            bool shouldTerminate = false;
            try
            {
                try
                {
                    if (!m_closed)
                    {
                        await WriteFrame(m_heartbeatFrame);
                    }
                }
                catch (Exception e)
                {
        ...
        public Task WriteFrame(OutboundFrame f)
        {
            return m_frameHandler.WriteFrame(f);
        }

        public Task WriteFrameSet(IList<OutboundFrame> f)
        {
            return m_frameHandler.WriteFrameSet(f);
        }

        ///<summary>API-side invocation of connection abort.</summary>
        public Task Abort()
        {
            return Abort(Timeout.Infinite);
        }

        ///<summary>API-side invocation of connection abort.</summary>
        public Task Abort(ushort reasonCode, string reasonText)
        {
            return Abort(reasonCode, reasonText, Timeout.Infinite);
        }

        ///<summary>API-side invocation of connection abort with timeout.</summary>
        public Task Abort(int timeout)
        {
            return Abort(Constants.ReplySuccess, "Connection close forced", timeout);
        }

        ///<summary>API-side invocation of connection abort with timeout.</summary>
        public Task Abort(ushort reasonCode, string reasonText, int timeout)
        {
        ...
        public Task Close()
        {
            return Close(Constants.ReplySuccess, "Goodbye", Timeout.Infinite);
        }

        ///<summary>API-side invocation of connection.close.</summary>
        public Task Close(ushort reasonCode, string reasonText)
        {
            return Close(reasonCode, reasonText, Timeout.Infinite);
        }

        ///<summary>API-side invocation of connection.close with timeout.</summary>
        public Task Close(int timeout)
        {
            return Close(Constants.ReplySuccess, "Goodbye", timeout);
        }

        ///<summary>API-side invocation of connection.close with timeout.</summary>
        public Task Close(ushort reasonCode, string reasonText, int timeout)
        {
        ...
        public async Task<IModel> CreateModel()
        {
            ...
            await model._Private_ChannelOpen("");
            return model;
        }
        ...
        protected async Task StartAndTune()
        {
            var connectionStartCell = new TaskCompletionSource<ConnectionStartDetails>();
            ...
            await m_frameHandler.SendHeader();

            var connectionStart = await connectionStartCell.Task;
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (challenge == null)
                    {
                        res = await m_model0.ConnectionStartOk(m_clientProperties,
                    ...
                    {
                        res = await m_model0.ConnectionSecureOk(response);
                    }
            ...
            await m_model0.ConnectionTuneOk(channelMax,
                frameMax,
                heartbeat);
