...
namespace RabbitMQ.Client.MessagePatterns
{
    ...
    public class Subscription : ISubscription
    {
        ...
        public async Task Subscribe(string consumerTag)
        {
            ...
            {
                ConsumerTag = await Model.BasicConsume(QueueName, AutoAck, m_consumer);
            }
            else
            {
                ConsumerTag = await Model.BasicConsume(QueueName, AutoAck, consumerTag, m_consumer);
            }
        ...
        ///</remarks>
        public async Task Ack(BasicDeliverEventArgs evt)
        {
            if (evt == null)
            {
                return;
            ...
            {
                await Model.BasicAck(evt.DeliveryTag, false);
            }
        ...
        public async Task Close()
        {
            try
            {
                ...
                if (shouldCancelConsumer)
                {
                    if (Model.IsOpen)
                    {
                        await Model.BasicCancel(ConsumerTag);
                    }
        ...
        ///null.</summary>
        public Task Nack(bool requeue)
        {
            return Nack(LatestEvent, false, requeue);
        }

        ///<summary>If LatestEvent is non-null, passes it to
        ///Nack(BasicDeliverEventArgs, multiple, requeue). Causes LatestEvent to become
        ///null.</summary>
        public Task Nack(bool multiple, bool requeue)
        {
            return Nack(LatestEvent, multiple, requeue);
        }
        ...
        /// channel, will lead to unpredictable behaviour
        ///</remarks>
        public async Task Nack(BasicDeliverEventArgs evt, bool multiple, bool requeue)
        {
            if (evt == null)
            {
                return;
            ...
            if (!AutoAck && Model.IsOpen)
            {
                await Model.BasicNack(evt.DeliveryTag, multiple, requeue);
            }
